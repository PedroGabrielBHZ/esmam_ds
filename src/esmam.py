...
class Esmam(Algorithm):
    ...
    def run(self) -> None:
        ...
        # Initialize stagnation
        # (would it be better initialising it in __init__?)
        self._stagnation = 0
    ...
    def _heuristic_map(self):
        ...
        n_items = self._dataset.get_number_of_items()
        self.__heuristic = np.fromiter(
            map(self._calculate_item_heuristic,
                range(self._dataset.get_number_of_items())),
            dtype=float
        )
    ...
    def _subgroup_search(self) -> rule.Rule:
        ...
        while ant <= self.__n_ants or \
                convergence <= self.__n_rules_for_convergence:
    ...
    def _subgroup_set_update(self, new_rule) -> bool:
        ...
        if new_rule.quality() > 1 - self._alpha:
            ...
            if can_add_rule:
                ...
                self.__uncovered_cases = np.setdiff1d(self.__uncovered_cases,
                                                      new_rule.get_cover())
            return True
        else:
            return False
...
if __name__ == "__main__":
    ...
    # TODO: Fazer com que essas funcoes sejam executadas automaticamente
